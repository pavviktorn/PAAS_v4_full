...
class MIDS(nn.Module):
    ...
    def unfreeze_vision_encoder(self, last_layers=2):
        # tf4.37 nested the encoder under .vision_model; tf5.13 flattened it onto the model.
        enc = getattr(self.image_encoder, "vision_model", self.image_encoder)
        for param in enc.encoder.layers[-last_layers:].parameters():
